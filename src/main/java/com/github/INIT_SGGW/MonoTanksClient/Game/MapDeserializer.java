    ...
    public Tile[][] deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                ...
                Tile tile = new Tile();
                tile.setVisible(visibility[i][j]);
                tile.setZoneIndex(zones[i][j] == null ? Optional.empty() : Optional.of(zones[i][j]));
                ...
                if (tileNode.size() > 0) {
                    ...
                    switch (type) {
                        ...
                        case "tank":
                            ...
                            turret.setDirection(turretDirection);
                            ...
                            turret.setBulletCount(bulletCount);
                            ...
                            turret.setTicksToRegenBullet(ticksToRegenBullet);
